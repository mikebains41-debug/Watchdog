...
DURATION_S = 120  # was fixed 600 iterations (~600s); standardized to match other modules
...
def get_ecc():
    try:
        out = subprocess.check_output(
            ["nvidia-smi", "--query-gpu=ecc.errors.corrected.volatile.total", "--format=csv,noheader,nounits"],
            text=True, timeout=3
        )
...
def main():
    ...
    while time.time() < start + DURATION_S:
        ...
        if mem is not None and ecc is not None and prev_mem is not None and prev_ecc is not None:
            if mem == prev_mem and ecc > prev_ecc * 2:
                alerts += 1
                log.write(json.dumps({
                    "detector": "D84_MEM_CLOCK_DESYNC",
                    "severity": "WARN",
                    "mem_clock": mem,
