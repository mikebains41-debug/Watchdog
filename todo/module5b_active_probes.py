...
"""
import time, datetime, json, os, subprocess

def now_iso():
    return datetime.datetime.now(datetime.timezone.utc).isoformat()

def probe_vram(size_mb=10):
    try:
        out = subprocess.check_output(["nvidia-smi", "--query-gpu=memory.used", "--format=csv,noheader,nounits"], text=True, timeout=3, stderr=subprocess.DEVNULL)
        before = float(out.strip())
        # Simulate allocation (placeholder)
        time.sleep(0.5)
        out = subprocess.check_output(["nvidia-smi", "--query-gpu=memory.used", "--format=csv,noheader,nounits"], text=True, timeout=3, stderr=subprocess.DEVNULL)
