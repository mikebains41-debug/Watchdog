...
POWER_HIGH_W=600.0; UTIL_LOW_PCT=20.0; D4_CONSEC=10; GHOST_MARGIN_W=40.0
PSTATE_JUMP=2; TEMP_CEIL_C=85.0; SAMPLE_HZ=1.0
...
def q(fields):
    try:
        out=subprocess.check_output(["nvidia-smi",f"--query-gpu={fields}",
            "--format=csv,noheader,nounits"],text=True,timeout=10,stderr=subprocess.DEVNULL)
...
def sample_gpus():
    ...
    for line in q("index,pstate,utilization.gpu,power.draw,memory.used,memory.total,temperature.gpu"):
        ...
        if len(p)<7: continue
        rows.append({"index":p[0],"pstate":p[1],"util":num(p[2]),"power":num(p[3]),
...
def compute_apps():
    try:
        ...
        for line in out.strip().splitlines():
            p=[x.strip() for x in line.split(",")]
            if len(p)>=4: a.append({"uuid":p[0],"pid":p[1],"name":p[2],"mem_mib":p[3]})
        return a
    except Exception: return []

def topo():
    try: return subprocess.check_output(["nvidia-smi","topo","-m"],text=True,
        timeout=10,stderr=subprocess.DEVNULL).strip()[:1200]
    except Exception: return None

def nvlink_err():
    try: return subprocess.check_output(["nvidia-smi","nvlink","-e"],text=True,
        timeout=10,stderr=subprocess.DEVNULL).strip()[:800]
    except Exception: return None

class Log:
    def __init__(s,path): s.f=open(path,"a"); s.path=path
    def emit(s,r): r["ts"]=now_iso(); s.f.write(json.dumps(r)+"\n"); s.f.flush()
...
def main():
    ...
    if not have("nvidia-smi"):
        print("nvidia-smi not found — run on the RunPod GPU pod."); sys.exit(1)
    stamp=datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    out=a.out or f"module1_nvml_{stamp}.jsonl"
    log=Log(out)
    prank={f"P{i}":i for i in range(16)}
    prev={}; d4=defaultdict(int); floor={}; samples=alerts=0
    ...
    warm=[]
    for _ in range(5):
        warm+=[g["power"] for g in sample_gpus() if g["power"] is not None]; time.sleep(0.5)
    base=(sum(warm)/len(warm)) if warm else 0.0
    t_end=time.time()+a.duration
    while time.time()<t_end:
        gpus=sample_gpus(); apps=compute_apps()
        by=defaultdict(list)
        for ap_ in apps: by[ap_["uuid"]].append(ap_)
        for g in gpus:
            gi=g["index"]; samples+=1; floor.setdefault(gi,base)
            if g["power"] and g["util"] is not None:
                if g["power"]>floor[gi]+GHOST_MARGIN_W and g["util"]<UTIL_LOW_PCT:
                    ...
                    log.emit({"detector":"D1_GHOST_POWER","severity":"WARN","gpu":gi,
                        "power_w":g["power"],"util":g["util"],"idle_floor_w":round(floor[gi],1),
                        "confidence":0.7,"note":"Power above idle floor while util low."})
                if g["power"]>POWER_HIGH_W and g["util"]<UTIL_LOW_PCT:
                    d4[gi]+=1
                    if d4[gi]==D4_CONSEC:
                        alerts+=1
                        log.emit({"detector":"D4_ZERO_SYSCALL_MINE","severity":"CRITICAL",
                            "gpu":gi,"power_w":g["power"],"util":g["util"],"consecutive":D4_CONSEC,
                            "confidence":0.85,"note":"High power + low util sustained — hidden kernel?"})
                else: d4[gi]=0
            ps=g["pstate"]
            ...
            if g["temp"] and g["power"] and g["util"] is not None:
                if g["temp"]>=TEMP_CEIL_C and g["power"]>POWER_HIGH_W and g["util"]<UTIL_LOW_PCT:
                    ...
                    log.emit({"detector":"D8_THERMAL_STRESS","severity":"CRITICAL","gpu":gi,
                        "temp_c":g["temp"],"power_w":g["power"],"util":g["util"],"confidence":0.8,
                        "note":"Heat+power at ceiling without matching compute."})
            owners=by.get(gi) or []
            if g["mem_used"] and g["mem_used"]>1024 and not owners:
                log.emit({"detector":"D7_ATTRIBUTION","severity":"WARN","gpu":gi,
