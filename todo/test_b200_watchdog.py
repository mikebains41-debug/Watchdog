...
# ═══════════════════════════════════════
# MODULES 76-80 — AI Agent Containment
# ═══════════════════════════════════════
section("module76.py — Agent Egress Containment")
import module76
test("IMDS endpoint 169.254.169.254 watched", lambda: "169.254.169.254" in module76.METADATA_ENDPOINTS)
test("ECS metadata endpoint watched", lambda: "169.254.170.2" in module76.METADATA_ENDPOINTS)
test("Launchpad services defined", lambda: len(module76.LAUNCHPAD_SERVICES) >= 8)
test("Exfil channels defined", lambda: len(module76.EXFIL_CHANNELS) >= 10)
test("ngrok in exfil channels", lambda: any("ngrok" in c for c in module76.EXFIL_CHANNELS))
test("is_private detects 10.x", lambda: module76.is_private("10.0.0.1"))
test("is_private rejects public", lambda: not module76.is_private("8.8.8.8"))
test("agent markers defined", lambda: "langchain" in module76.AGENT_MARKERS)
test("classify metadata endpoint", lambda: module76.classify_destination("169.254.169.254", None)[0] == "METADATA")
test("classify external", lambda: module76.classify_destination("8.8.8.8", None)[0] == "EXTERNAL")
test("detection only documented", lambda: "detection only" in inspect.getsource(module76.main))

section("module77.py — Agent Action Rate & Coordination")
import module77
test("ACTIONS_PER_MIN_CRIT = 200", lambda: module77.ACTIONS_PER_MIN_CRIT == 200)
test("SUSTAINED_HOURS_CRIT = 24", lambda: module77.SUSTAINED_HOURS_CRIT == 24)
test("REGULARITY_CV_MAX = 0.15", lambda: module77.REGULARITY_CV_MAX == 0.15)
test("SPAWN_STORM_THRESHOLD = 20", lambda: module77.SPAWN_STORM_THRESHOLD == 20)
test("pearson of identical = 1.0", lambda: abs(module77.pearson([1,2,3],[1,2,3]) - 1.0) < 0.001)
test("mean_std of constant = 0 std", lambda: module77.mean_std([5,5,5])[1] == 0.0)
test("is_agent_process detects langchain", lambda: module77.is_agent_process("python -m langchain.agent"))
test("is_agent_process rejects vim", lambda: not module77.is_agent_process("/usr/bin/vim notes.txt"))
test("find_shm_segments runs", lambda: isinstance(module77.find_shm_segments(), list))
test("17600 action scale referenced", lambda: "17,600" in inspect.getsource(module77))

section("module78.py — K8s Token Abuse")
import module78
test("SA token path correct", lambda: "/var/run/secrets/kubernetes.io/serviceaccount/token" in module78.SA_TOKEN_PATHS)
test("kubelet cred paths defined", lambda: len(module78.KUBELET_CRED_PATHS) >= 3)
test("system:node in privileged identities", lambda: any("system:node" in p for p in module78.PRIVILEGED_IDENTITIES))
test("cluster-admin in privileged identities", lambda: "cluster-admin" in module78.PRIVILEGED_IDENTITIES)
test("kubectl in cluster tools", lambda: "kubectl" in module78.CLUSTER_TOOLS)
test("crictl in cluster tools", lambda: "crictl" in module78.CLUSTER_TOOLS)
test("in_kubernetes returns bool", lambda: isinstance(module78.in_kubernetes(), bool))
test("decode_jwt_claims rejects garbage", lambda: module78.decode_jwt_claims("not.a.jwt") is None or isinstance(module78.decode_jwt_claims("not.a.jwt"), dict))
test("count_containers runs", lambda: isinstance(module78.count_containers(), dict))
test("token values never logged", lambda: "never logged" in inspect.getsource(module78.main))

section("module79.py — Registry Proxy Integrity")
import module79
test("Artifactory fixed version 7.161.15", lambda: module79.PROXY_FIXED_VERSIONS["artifactory"][0] == "7.161.15")
test("CVE-2026-65617 cited", lambda: "65617" in inspect.getsource(module79))
test("CVE-2026-65924 SSRF cited", lambda: "65924" in inspect.getsource(module79))
test("CVE-2026-66014 privesc cited", lambda: "66014" in inspect.getsource(module79))
test("version_tuple parses correctly", lambda: module79.version_tuple("7.161.15") == (7, 161, 15))
test("version comparison works", lambda: module79.version_tuple("7.161.14") < module79.version_tuple("7.161.15"))
test("known upstreams defined", lambda: "pypi.org" in module79.KNOWN_UPSTREAMS)
test("traversal patterns defined", lambda: len(module79.TRAVERSAL_PATTERNS) >= 5)
test("SSRF patterns defined", lambda: len(module79.SSRF_PATTERNS) >= 5)
test("find_proxy_processes runs", lambda: isinstance(module79.find_proxy_processes(), list))
test("anonymous access check present", lambda: callable(module79.check_anonymous_access))

section("module80.py — Credential Harvest & Reuse")
import module80
test("MASS_ACCESS_CRITICAL = 12", lambda: module80.MASS_ACCESS_CRITICAL == 12)
test("CROSS_SERVICE_THRESHOLD = 2", lambda: module80.CROSS_SERVICE_THRESHOLD == 2)
test("AWS credentials path watched", lambda: any(".aws/credentials" in p for p in module80.CREDENTIAL_GLOBS))
test("kube config watched", lambda: any(".kube/config" in p for p in module80.CREDENTIAL_GLOBS))
test("SSH keys watched", lambda: any(".ssh/id_" in p for p in module80.CREDENTIAL_GLOBS))
test("aws in cloud CLI", lambda: "aws" in module80.CLOUD_CLI)
test("gcloud in cloud CLI", lambda: "gcloud" in module80.CLOUD_CLI)
test("tailscale in mesh tools", lambda: "tailscale" in module80.MESH_TOOLS)
test("wireguard in mesh tools", lambda: any("wireguard" in t or t == "wg" for t in module80.MESH_TOOLS))
test("zerotier in mesh tools", lambda: any("zerotier" in t for t in module80.MESH_TOOLS))
test("mesh interface prefixes defined", lambda: "wg" in module80.MESH_INTERFACE_PREFIXES)
test("is_private detects 192.168", lambda: module80.is_private("192.168.1.1"))
test("resolve_credential_paths runs", lambda: isinstance(module80.resolve_credential_paths(), set))
