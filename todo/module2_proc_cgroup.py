...
"""
import os, sys, time, datetime, json, subprocess, re
...
def check_cron():
    ...
    for loc in ['/etc/crontab', '/etc/cron.d/', '/var/spool/cron']:
        if os.path.exists(loc):
            out.append(loc)
    try:
        p = subprocess.check_output(['crontab', '-l'], stderr=subprocess.DEVNULL, text=True)
...
def main():
    ...
    while time.time() < start + 120:
        ...
        for p in procs:
            if p['ppid'] == '1' and p['vmrss'] > 500 * 1024:
                alerts += 1
                log.write(json.dumps({
        ...
        if cpu_shares is not None and cpu_shares > 0 and cpu_shares < 0.01:
            alerts += 1
            log.write(json.dumps({
        ...
        if io_r and io_w and io_r + io_w > 100 * 1024 * 1024 * 1024:
            log.write(json.dumps({
                "detector":"D12_IO_BURST",
                "severity":"INFO",
                "read_bytes":io_r,
                "write_bytes":io_w,
                "confidence":0.5,
