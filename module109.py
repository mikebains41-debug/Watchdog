"""Module 109 — 16-Qubit GHZ Scaling Test (Rigetti)
Extends module106 (3q, 79.10%) and module107 (8q, 18.29%) to 16 qubits
— a third real data point for the entanglement scaling curve."""
...
def build_ghz16():
    lines = ["OPENQASM 2.0;", 'include "qelib1.inc";', "qreg q[16];", "creg c[16];", "h q[0];"]
    for i in range(15): lines.append(f"cx q[{i}],q[{i+1}];")
    for i in range(16): lines.append(f"measure q[{i}] -> c[{i}];")
    return "\n".join(lines) + "\n"
...
if __name__ == "__main__":
    ...
    print(f"Provider: {provider.provider_name}\n--- 16-qubit GHZ to {BACKEND} ---")
    qasm = build_ghz16()
    jid = submit_with_recovery(provider, qasm, 8192, BACKEND, "watchdog_ghz16_rigetti")
    print(f"Job ID: {jid}\nPolling...")
    ...
    if status in ("Completed", "Done"):
        ...
        z16 = "0"*16; o16 = "1"*16
        corr = counts.get(z16, 0) + counts.get(o16, 0)
        frac = corr/total if total else 0
        print(f"\n16-qubit GHZ correlation: {frac:.4f}  (3q=79.10%, 8q=18.29%, 16q={frac*100:.2f}%)")
        result.update({"counts": counts, "ghz16_correlated_fraction": round(frac,4), "shots": total})
    else:
        print(f"INCOMPLETE — {status}")
    OUTPUT_DIR = "/data/data/com.termux/files/home/Watchdog-quantum-collab/open_quantum_results"
    os.makedirs(OUTPUT_DIR, exist_ok=True)
    with open(os.path.join(OUTPUT_DIR, "module109_ghz16_rigetti_result.json"), "w") as f:
