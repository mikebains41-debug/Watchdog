"""Module 109 — 5-Qubit GHZ Scaling Test (Rigetti)
Extends module106 (3q, 79.10%) and module107 (8q, 18.29%) to 5 qubits
...
def build_ghz16():
    lines = ["OPENQASM 2.0;", 'include "qelib1.inc";', "qreg q[5];", "creg c[5];", "h q[0];"]
    for i in range(4): lines.append(f"cx q[{i}],q[{i+1}];")
    for i in range(5): lines.append(f"measure q[{i}] -> c[{i}];")
    return "\n".join(lines) + "\n"
...
if __name__ == "__main__":
    ...
    qasm = build_ghz16()
    jid = submit_with_recovery(provider, qasm, 2048, BACKEND, "watchdog_ghz5_rigetti")
    print(f"Job ID: {jid}\nPolling...")
    ...
    with open(os.path.join(OUTPUT_DIR, "module109b_ghz5_rigetti_result.json"), "w") as f:
